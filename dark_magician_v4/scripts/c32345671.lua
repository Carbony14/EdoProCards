...
function s.chaosfilter(c)
	return ((c:IsRitualMonster() and c:IsSetCard(0xcf)) or c:IsCode(30208479))
		and c:IsAbleToHand()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		-- If "Dark Magician" is on field or in GY, optionally add 1 "Chaos" Ritual Monster
		if Duel.IsExistingMatchingCard(s.dmfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil)
			and Duel.IsExistingMatchingCard(s.chaosfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil)
			and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then

			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g2=Duel.SelectMatchingCard(tp,s.chaosfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
			if #g2>0 then
